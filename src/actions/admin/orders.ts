...
function serializeOrder(order: Order) {
  return {
    ...
    baseInfo_totalAmount: order.baseInfo_totalAmount ? Number(order.baseInfo_totalAmount) : null,
    productItems_productID: order.productItems_productID ? order.productItems_productID.toString() : null,
    productItems_quantity: order.productItems_quantity ? order.productItems_quantity.toString() : null,
    productItems_price: order.productItems_price ? Number(order.productItems_price) : null,
    productItems_itemAmount: order.productItems_itemAmount ? Number(order.productItems_itemAmount) : null,
...
function groupOrdersByBaseId(orders: any[]) {
  ...
  orders.forEach(order => {
    ...
    if (!groupedOrders.has(baseId)) {
      ...
      groupedOrders.set(baseId, {
        ...
        baseInfo_buyerContact_companyName: order.baseInfo_buyerContact_companyName,
...
function serializeStats(stats: any) {
  return {
    ...
    topBuyers: stats.topBuyers.map((buyer: any) => ({
      baseInfo_buyerID: buyer.baseInfo_buyerID,
      baseInfo_buyerContact_name: buyer.baseInfo_buyerContact_name,
      baseInfo_buyerContact_companyName: buyer.baseInfo_buyerContact_companyName,
      _count: buyer._count,
      _sum: {
        baseInfo_totalAmount: buyer._sum.baseInfo_totalAmount ? Number(buyer._sum.baseInfo_totalAmount) : 0
      }
    })),
    topProducts: stats.topProducts.map((product: any) => ({
      ...
      _sum: {
        productItems_quantity: product._sum.productItems_quantity ? product._sum.productItems_quantity.toString() : '0',
        productItems_itemAmount: product._sum.productItems_itemAmount ? Number(product._sum.productItems_itemAmount) : 0
      }
    }))
  };
}
...
export async function getOrdersWithProducts() {
  try {
    ...
    try {
      ...
      const groupedBasicOrders = groupOrdersByBaseId(basicOrders);
      return groupedBasicOrders;
    } catch (fallbackError) {
      console.error('Fallback query also failed:', fallbackError);
      throw new Error('Failed to fetch orders - database connection or data integrity issue');
    }
  }
}
...
export async function getOrderDetails(orderId: string) {
  try {
    ...
    const groupedOrders = groupOrdersByBaseId(orderItems);
...
export async function getOrdersInBatches(batchSize: number = 50, offset: number = 0) {
  try {
    ...
    console.error(`Error fetching orders batch (offset: ${offset}, size: ${batchSize}):`, error);
...
export async function bulkUpdateOrderApproval(orderIds: string[], approved: boolean) {
  try {
...
export async function getOrderStats() {
  try {
    ...
    const total = new Set(uniqueOrderCounts.map(item => item.baseInfo_id.toString())).size;
    ...
    const [totalValue, topBuyers, topProducts] = await Promise.all([
      ...
      prisma.orders_by_products.groupBy({
        by: ['baseInfo_id'],
        _sum: {
          baseInfo_totalAmount: true,
        },
        ...
        return results.reduce((sum, order) => sum + (Number(order._sum.baseInfo_totalAmount) || 0), 0);
      }),
      
      // Top buyers by order count (unique orders only)
      prisma.orders_by_products.groupBy({
        by: ['baseInfo_buyerID', 'baseInfo_buyerContact_name', 'baseInfo_buyerContact_companyName', 'baseInfo_id'],
        _sum: {
          baseInfo_totalAmount: true,
        },
        where: {
        ...
        results.forEach(result => {
          ...
          if (!buyerMap.has(buyerId)) {
            buyerMap.set(buyerId, {
              ...
              baseInfo_buyerContact_name: result.baseInfo_buyerContact_name,
              baseInfo_buyerContact_companyName: result.baseInfo_buyerContact_companyName,
              _count: { id: 0 },
          ...
          if (!buyer.orderIds.has(result.baseInfo_id.toString())) {
            ...
            buyer._count.id++;
            buyer._sum.baseInfo_totalAmount += Number(result._sum.baseInfo_totalAmount || 0);
          }
        });
